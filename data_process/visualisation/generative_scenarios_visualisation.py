...
def update_plot(save_frame=False, frame_dir=None, frame_idx=0):
    ...
    if save_frame and frame_dir is not None:
        frame_path = os.path.join(frame_dir, f"frame_{frame_idx:05d}.png")
        fig.savefig(frame_path)

def save_all_frames_as_images(frame_dir):
    global timestamp
    os.makedirs(frame_dir, exist_ok=True)
    frame_idx = 0
    for ts in range(int(timestamp_min), int(timestamp_max + 1), dataset_types.DELTA_TIMESTAMP_MS):
        timestamp = ts
        update_plot(save_frame=True, frame_dir=frame_dir, frame_idx=frame_idx)
        frame_idx += 1
...
##############################################################     file_path          ##########################################################################
    ...
    track_file_name = os.path.join(target_dir, f'{2}.csv')
    ...
    def create_gif_from_frames(frame_dir, output_path, fps=10):
        images = []
    ...
    # 保存帧图像的目录
    frame_output_dir = os.path.join(root_dir, "output_frames")
    save_all_frames_as_images(frame_output_dir)

    # 生成 GIF
    gif_output_path = os.path.join(root_dir, "scene_playback.gif")
    create_gif_from_frames(frame_output_dir, gif_output_path, fps=10)
